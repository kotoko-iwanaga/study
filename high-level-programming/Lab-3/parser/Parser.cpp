...
class Parser {
 ...
 private:
  ...
  void parser_method_arguments_and_body() {
    ...
    this->require(TokenType::lsquirly);  // {
    this->parse_method_body();           // Alexandr Segreevich will check code inside
    this->require(TokenType::rsquirly);  // }
  }
  ...
  void parse_method_body() {
    while (this->tok().type != TokenType::rsquirly) {  // }
      this->eat();                                     // Alexandr Segreevich will check code inside
      if (this->eof())
        this->require(TokenType::rsquirly);  // throw error
    }
    std::cout <<  "Alexandr Segreevich will check code inside method" << std::endl;
  }
